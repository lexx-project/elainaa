...
`.trim()
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options = {}, smlcap = { smlcap : true }) {
                ...
                let thumbs = ['thumb-1', 'thumb-2', 'thumb-3', 'thumb-4', 'thumb-5']
                    return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { text: isSmlcap ? conn.smlcap(text, smlcap.except ? smlcap.except : false) : text, contextInfo: {
    forwardingScore: 1,
    isForwarded: true,
    forwardedNewsletterMessageInfo: {
      newsletterJid: '120363314171908895@newsletter',
      serverMessageId: 103,
      newsletterName: '⌜ Elaina - Ai ⌟'
    },
                externalAdReply: {
                    ...
                    body: '',
                    mediaType: 1,
                    thumbnail: fs.readFileSync('./media/' + thumbs.getRandom() + '.jpg'),
        ...
        adReply: {
            async value(jid, text, title = '', body = '', buffer, source = '', quoted, large = true, options = {}, smlcap = { smlcap: true }) {
                ...
                return conn.sendFile(jid, data, null, isSmlcap ? conn.smlcap(text, smlcap.except ? smlcap.except : false) : text, quoted)
            },
            enumerable: true
        },
        /*
