...
const checkSession = async (ZetInc) => {
    if (ZetInc.authState.creds.registered) {
        console.log(chalk.cyanBright(
            figlet.textSync(`ZenOfficial`, {
        ...
        console.log(chalk.bgBlack.blueBright.bold("   ≼≽ Welcome Back, Zack!!! "))
        console.log("")
        console.log("")
        console.log("")
...
const fetchKeyFromGithub = async () => {
    const githubUrl = 'https://raw.githubusercontent.com/ZackDevOfficial/ZenOfficial/main/ZenOfficialcode.json'
    try {
        const response = await fetch(githubUrl)
        const data = await response.json()
        return data.password
    } catch (err) {
        console.error("Gagal mengambil password dari Github:", err)
        process.exit()
    }
}

const centerText = (txt, width) => {
...
const verifyPassword = async () => {
    const githubPassword = await fetchKeyFromGithub()
    const userPassword = await question('[ ! ] Input Password:\n')

    const termWidth = process.stdout.columns

    if (userPassword !== githubPassword) {
        const errorText = figlet.textSync('Wrong!', {
            horizontalLayout: 'default',
            verticalLayout: 'default'
        })
        console.log(chalk.red(centerText(errorText, termWidth)))
        console.log(chalk.red(centerText('Access Denied. (⁠눈⁠‸⁠눈⁠)', termWidth)))
        process.exit(1);
    } else {
        const successText = figlet.textSync('Welcome!', {
            horizontalLayout: 'default',
            verticalLayout: 'default'
        })
        console.log(chalk.green(centerText(successText, termWidth)))
        console.log(chalk.green(centerText('Access Accepted (⁠｡⁠•̀⁠ᴗ⁠-⁠)⁠✧', termWidth)))
    }
}
...
async function connectionUpdate(update) {
    ...
    if (receivedPendingNotifications) console.log(chalk.yellow('nggu Pesan Baru'))
