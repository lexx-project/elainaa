...
const handler = async (m, { conn, usedPrefix, text, args, command }) => {
  ...
  if (!count) {
    try {
      const tio = await fetch(`https://api.lolhuman.xyz/api/pinterest2?apikey=RyAPI&query=${query}`);
    ...
    try {
      let url = `https://api.lolhuman.xyz/api/pinterest2?apikey=RyAPI&query=${query}`;
...
handler.tags = ['tool','random','internet'];
